...
-- ลบ for loop เดิมทิ้งทั้งหมด แล้วใส่อันนี้
...
local function autoScale()
    ...
    if isMobile then
        local scale = math.min(vp.X / 538, vp.Y / 288) * 0.85
        main.Size = UDim2.new(0, 538 * scale, 0, 288 * scale)
    else
        local scale = math.min(vp.X / 1280, vp.Y / 720)
