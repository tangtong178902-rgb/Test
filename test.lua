...
local function ShowKeyUI(initialMessage, initialKey)
    ...
    verifyBtn.MouseButton1Click:Connect(function()
        ...
        if res.success then
            if writefile then writefile(FileName, key) end -- บันทึกลงไฟล์เฉพาะเมื่อผ่าน UI
...
local function Initialize()
    ...
    _G.Premium = nil -- ล้างค่า global ทันที

    -- 1. ถ้ามีการใส่ _G.Premium (ให้ความสำคัญเป็นอันดับแรก แต่ไม่บันทึกลงไฟล์)
    if inputKey and inputKey ~= "" then
        print("[NEXIT] Validating Premium Bypass Key...")
        local res = Validate(inputKey, true)
        if res.success and res.isPremium then
            LoadPremiumHub()
            return -- จบการทำงาน (ไม่ยุ่งกับไฟล์ ไม่เปิด UI)
        else
            warn("❌ [NEXIT] _G.Premium invalid: " .. res.message)
            ShowKeyUI("Premium Key Error: " .. res.message, inputKey)
            return -- จบการทำงาน (เพื่อให้ผู้ใช้แก้ใน UI)
        end
    end

    -- 2. ถ้าไม่มี _G ให้เช็ค Saved Key ในเครื่อง
    local savedKey = (isfile and isfile(FileName)) and readfile(FileName)
    if savedKey and savedKey ~= "" then
        print("[NEXIT] Validating saved key...")
