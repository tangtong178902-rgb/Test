...
-- ย้ายการสร้าง UI มาไว้ในฟังก์ชัน เพื่อให้เรียกใช้เฉพาะตอนที่จำเป็นเท่านั้น
local function ShowKeyUI(initialMessage)
    ...
    status.Text = initialMessage or "Verification Required"
...
local function Initialize()
    -- ขั้นตอนที่ 1: ตรวจสอบ _G.Premium (บังคับพรีเมียม)
    if _G.Premium and _G.Premium ~= "" then
        print("[NEXIT] Checking provided Premium Key...")
        local res = Validate(_G.Premium, true)
        if res.success and res.isPremium then
            LoadPremiumHub()
            return -- จบการทำงานทันที
        else
            -- ถ้าใส่ _G.Premium มาแต่คีย์ผิด ให้แจ้งเตือนและหยุดสคริปต์ (ไม่เปิด UI)
            warn("❌ [NEXIT] Premium Key is invalid: " .. res.message)
            return 
        end
    end

    -- ขั้นตอนที่ 2: ตรวจสอบ Saved Key (คีย์ที่เคยเซฟไว้)
    local savedKey = nil
    ...
    -- ขั้นตอนที่ 3: ถ้าไม่มีคีย์ไหนผ่านเลย ค่อยเปิด UI
    print("[NEXIT] No valid key found. Opening UI...")
    ShowKeyUI()
end
