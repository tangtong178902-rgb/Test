...
local function Initialize()
    -- ดึงค่าจาก _G แล้วล้างค่าทิ้งทันที เพื่อป้องกันการค้างในหน่วยความจำ
    local inputPremiumKey = _G.Premium
    _G.Premium = nil 

    -- 1. ตรวจสอบ Premium จาก _G (Priority สูงสุด)
    if inputPremiumKey and inputPremiumKey ~= "" then
        ...
        if res.success and res.isPremium then
            if writefile then writefile(FileName, inputPremiumKey) end -- เซฟทับถ้าคีย์ใหม่ใช้ได้
            LoadPremiumHub()
            return
        else
            -- ถ้าคีย์ใน _G ผิด ให้ข้ามไปเช็คไฟล์เซฟ เผื่อมีคีย์เก่าที่ยังใช้ได้
            warn("❌ [NEXIT] Provided Key is invalid, checking saved key...")
        end
    end

    -- 2. ตรวจสอบ Saved Key ในเครื่อง
